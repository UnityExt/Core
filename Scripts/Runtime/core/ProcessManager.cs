...
namespace UnityExt.Core {
    ...
    public class ProcessManager : ScriptableObject {
        ...
        /// <summary>
        /// Helper for threading and playing flag
        /// </summary>
        static private object lock_playing = new object();
        ...
        internal void PushProcess(Process p_process) {            
            Process p = p_process;
            if (p == null) return;
            //Lastly clear the process reference if its the process being returned otherwise keep it (activity being refreshed/restarted)
            if (p.activity != null) if(p.activity.process == p_process) p.activity.process = null;
        ...
        internal void AddProcess(Process p_process,bool p_force) {
            ...
            if (p.state == ProcessState.Start) return;
            p.state = ProcessState.Add;
        ...
        /// </summary>
        /// <param name="p_process"></param>
        internal void RemoveProcess(Process p_process,bool p_force) {
            Process p = p_process;
            if (p == null) return;
            if (p.state == ProcessState.Remove) return;
            if (p.state == ProcessState.Stop  ) return;
            p.state = ProcessState.Remove;            
            //Debug.Log($"Process> {p.activity} REMOVE - {p.state} @ [{p.activity?.process?.pid}] [{p.pid}]");            
            for (int i = 0;i < units.Count;i++) {
                ProcessUnit it = units[i];
                if ((it.context & p.context) == 0) continue;
                it.Remove(p_process,p_force);
            }            
        }
        ...
        protected void OnValidate() {
            ...
            while (stk.Count > pp.Length) stk.RemoveAt(0);
